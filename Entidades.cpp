...
// void entidade::setAlphaValue(double a){
//     alphaValue = a;
// }
// true se |vel| != 0
...
void entidade::mexe(posicao _posLim, velocidade _vel, float speedMag){
    estado = Movimento;
    setVel(_vel);
    vMag = speedMag;
    setPosLim(_posLim);
...
void miniFluffy::setRotacao(velocidade r){
    rotacao->vx = r.vx;
    rotacao->vy = r.vy;
    rotacao->vz = r.vz;
}
...
void miniFluffy::SetMiniFluffy(posicao blockPos, int blockType){
    estado = Movimento;
    estado2 = Normal;

    posicao pos;
    pos.y = blockPos.y;
    if (blockType == FluffySpawnRight) {
        pos.x = blockPos.x;
        pos.z = blockPos.z - 1;
    } else if (blockType == FluffySpawnDown) {
        pos.x = blockPos.x + 1;
        pos.z = blockPos.z;
    } else if (blockType == FluffySpawnLeft) {
        pos.x = blockPos.x;
        pos.z = blockPos.z + 1;
    } else if (blockType == FluffySpawnUpwards) {
        pos.x = blockPos.x - 1;
        pos.z = blockPos.z;
    }
    ...
    setRotacao(vel);
    // setVel(velocidade(-1,0,0));

    // setRotacao(velocidade(-1,0,0));
}

void miniFluffy::Rotaciona(bool clockwise){
    if (clockwise)
        iRotacao = (iRotacao+3)%4;
    else
        iRotacao = (iRotacao+1)%4;
...
bool miniFluffy::colisaoVitoria(){
    return true;
}
...
block::block(posicao p, int t, double alpha, int stage) {
    pos= new posicao(p);
    vel = new velocidade();
    posLim = new posicao();
    prox = nullptr; 
    ...
    levelStage = stage;
    dissapearing = false;
    miniFluffysSpawned = 0;
}

block::block(posicao p, tipoBloco t, double alpha, int stage) {
    pos= new posicao(p);
    vel = new velocidade();
    posLim = new posicao();
    prox = nullptr; 
...
void LLBlocos::AdicionaBloco(posicao p, int t, int stageLevel, double alpha){
    block * Bloco = new block(p, t, alpha, stageLevel);
    AdicionaBloco(Bloco);
}

void LLBlocos::AdicionaBloco(posicao p, tipoBloco t){
    block * Bloco = new block(p, t, 1, 1);
    AdicionaBloco(Bloco);
}
...
bool andar::AndarCompleto(int stage){
    if (stage == Rapunzel)
        for (block *b = Lista->lista; b != nullptr; b = b->prox)
            if (b->tipo==FinalFixo || b->tipo==FinalMovel) return false;
    if (stage == Sokkoban)
        for (block *b = Lista->lista; b != nullptr; b = b->prox)
            if (b->tipo==FinalTargetIncompleto) return false;
    if (stage == MiniFluffys)
        return false;
    return true;
}

bool andar::coordenadaOcupada (posicao p){ return Lista->contem(p); }
void andar::AdicionaBloco(posicao p, int t, int stageLevel, double alpha) { Lista->AdicionaBloco(p,t, stageLevel, alpha); }
...
andar::andar(string s, int n){
    ...
    while (i < s.size()) {
        ...
        if (c == '/' || c == '-') {
            ...
            c = s[++i];
            if (c != ',' && c != '/') { std::cout << "Wrong Format! Expected ','." << "Got " << c << "\n"; }
            ...
            if (c != ' ' && c != '/') { std::cout << "Wrong Format! Expected ' '." << "Got " << c << "\n"; }
            if (block_type > 0){
                if (stageLevel == 1) alpha = 1.0f;
                else alpha = 0.0f;
                AdicionaBloco(p, block_type, stageLevel, alpha);
            }
            p.z+=1;
        }
        i++;
...
bool torre::FaseCompleta(int stage){
    for (andar *a = primeiroAndar; a != nullptr; a = a->prox)
        if (! a->AndarCompleto(stage)) return false;
    return true;
}
...
void torre::SetTorre(string filename){
    ...
    // cria matriz andar aparti¶r de arquivo
...
tipoColisao torre::ChecaColisaoPlayer(player * pl, torre * tr){
    ...
    if (pAprox == posicao(pl->pos)) // se esta proximo o bastante para poder interagir
    {
        ...
        if (FaseCompleta(tr->stages[pl->levelStage-1])) {
            pl->levelStage++;
            return ColisaoDeApoio;
        }
